<template>
  <div
    class="rounded-full group pl-0.5 pr-1 py-0.5 text-gray-900 bg-white border-2
    border-primary-500 font-body font-normal inline-flex items-center
    align-center w-max cursor-pointer transition duration-300 ease outline-violet"
  ...
  >
    ...
    <slot name="close">
      <button
        ...
        :class="[chipsButtonSpacing, hideButton]"
...
export default {
  ...
  setup(props) {
    ...
    const hideButton = computed(() => (!props.value ? 'hidden' : 'block'));
    return {
      chipsSize,
      chipsIconSize,
      chipsFontSize,
