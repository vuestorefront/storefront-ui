<template>
  <div
    v-if="!useBadgeProp"
    :class="_classStringToObject(`${tagVariants} ${tagSizes}`)"
  >
    <div
      v-if="$slots.prefix"
      :class="
        _classStringToObject(
          `${useSizeProp === VsfTagSizes.sm ? 'w-4 h-4' : 'w-5 h-5'} mr-1 flex`
        )
      "
    >
      <slot name="prefix" />
    ...
    <div
      ...
      :class="
        _classStringToObject(
          `${useSizeProp === VsfTagSizes.sm ? 'w-4 h-4' : 'w-5 h-5'} ml-1 flex`
        )
      "
    >
      <slot name="suffix" />
