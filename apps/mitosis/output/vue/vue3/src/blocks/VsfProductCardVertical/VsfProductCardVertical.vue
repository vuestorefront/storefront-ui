  ...
  <div
    :class="
      _classStringToObject(
        `${productCardWidth} overflow-hidden border border-gray-200 rounded-md hover:shadow-lg`
      )
    "
  >
    <div class="relative">
      ...
      <div class="absolute bottom-0 left-0 right-0 flex items-end m-2">
        ...
        <vsf-button
          ...
          :icon="true"
        >
          <vsf-icon-favorites-outline size="sm"></vsf-icon-favorites-outline>
        </vsf-button>
      </div>
    </div>
    <div
...
export default {
  ...
  computed: {
    ...
    isXs() {
      return this.useSizeProp === VsfProductCardVerticalSizes.xs;
    },
    isSm() {
      return this.useSizeProp === VsfProductCardVerticalSizes.sm;
    },
    isBase() {
      return this.useSizeProp === VsfProductCardVerticalSizes.base;
    },
    isLg() {
      return this.useSizeProp === VsfProductCardVerticalSizes.lg;
    },
    labelClasses() {
      return classStringFromArray([
        ...
        !this.isLg && "px-4 pt-4",
        !this.useWithButtonProp && this.isBase && "px-4 pt-4",
        !this.useWithButtonProp && this.isSm && "px-2 pt-2",
        !this.useWithButtonProp && this.isXs && "px-2 pt-2",
      ]);
    },
    priceClasses() {
      return classStringFromArray([
        "font-bold font-headings",
        (this.isXs || this.isSm) && "text-xs leading-4",
        (this.isBase || this.isLg) && "text-lg leading-6",
