  ...
  <div class="custom-block mt-16">
    <div class="grid grid-cols-12 gap-8">
      <RouterLink
        ...
        :to="generateComponentPath(type, componentName, showcase)"
      >
...
export default {
  ...
  computed: {
    components() {
      const files = this.showcase ? showcases : components;
  ...
  methods: {
    componentDescription(componentName) {
      const componentPath = `/${this.type}/${this.showcase ? 'showcases' : 'components'}/${componentName.replace('Vsf', '').toLowerCase()}.html`;
