        ...
        {categories.map(({ label, subcategories }) => (
          <div class="grid grid-cols xs:pb-4" key={label}>
            ...
            {subcategories?.map(({ subcategoryLabel, link }) => (
              <SfListItem class="py-2 !bg-transparent typography-text-sm font-body" key={subcategoryLabel}>
                <SfLink
      ...
      <div class="py-10 md:flex md:mx-auto max-w-[1536px]">
        {contactOptions.map(({ label, icon: Icon, link, details }) => (
          <div class="mx-auto my-4 text-center" key={label}>
            ...
            {details?.map((option) => (
              <p class="leading-5 typography-text-sm text-neutral-600 font-body" key={option}>
                {option}
              </p>
            ))}
          </div>
