...
export default component$(() => {
  ...
  const handleFilterSelection = $((val: string) => {
    selectedFiltersSignal.value.indexOf(val) > -1
      ? (selectedFiltersSignal.value = [...selectedFiltersSignal.value.filter((value) => value !== val)])
        ...
        <>
          ...
          >
            <div class="flex justify-between p-2 mb-2">
              <p class="mb-2 font-medium typography-headline-5">{section.summary}</p>
              <SfIconChevronLeft
                class={`text-neutral-500 ${isAccordionItemOpen(section.id) ? 'rotate-90' : '-rotate-90'}`}
              />
            </div>
            <div q:slot="detail">
              {section.type === 'size' && (
              ...
              {section.type === 'color' &&
                section.details.map(({ id, label, value, counter }) => (
                  <SfListItem
                    ...
                    as="label"
                    size="sm"
                    class={`px-1.5 bg-transparent hover:bg-transparent ${selectedFiltersSignal.value.includes(value) ? 'font-medium' : ''}`}
