import { SfButton, SfDrawer, SfDrawerPlacement, SfDrawerProps } from '@storefront-ui/qwik';
import { $, QwikChangeEvent, component$, useSignal } from '@builder.io/qwik';

export default component$(() => {
  const openSignal = useSignal(false);
  const placementSignal = useSignal<SfDrawerProps['placement']>(SfDrawerPlacement.left);
  ...
  return (
    <>
      <label>
        Placement
        <select onChange$={changePlacement} class="border-2 border-primary-700 mx-2 p-2 rounded">
      ...
      <SfDrawer
        ...
        ${
          placementSignal.value === SfDrawerPlacement.left || placementSignal.value === SfDrawerPlacement.right
            ? 'max-w-[370px]'
            : ''
        }`}
      >
