...
::: tip
Sometimes Vue orders rendered CSS differently between dev and production builds. To overcome this issue, use add `body` to your selector when overwriting global variables.
:::
...
```scss
@include generate-color-variants(--_c-green-primary, #018937);
```
...
>
  <template #prev="{ isDisabled, go, prev }">
    <button @click="go(prev)" :disabled="isDisabled">prev</button>
  </template>
  <template #next="{ isDisabled, go, next }">
    <button @click="go(next)" :disabled="isDisabled">next</button>
    </template>
</SfPagination>
```
