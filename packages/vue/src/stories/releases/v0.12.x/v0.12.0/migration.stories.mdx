...
so now you can configure it by two css custom properties: `--input-label-transform` to set initial state and `--input-label-translate3d` to set final state of the label.
It is recommended to use `translate3d` property because of performance reasons.
Similar chabes was made in SfComponentSelect, SfSelect. 

There is also `--input-bar-transform` property added to customize transition of bottom bar and it is set to use `scale3d` property also because of performance reasons. 
