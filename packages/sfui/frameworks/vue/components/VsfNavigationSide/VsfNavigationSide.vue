  ...
  >
    <slot name="navigation">
      <VsfNavbarTop filled class="vsf-navigation-side__navbar">
        ...
        <a :href="logoLink" :aria-label="logoAriaLabel" class="flex items-center text-white mx-0 h-8 md:h-10 lg:h-7">
          <VsfIconLogo viewBox="0 0 205 28" class="!w-[12.5rem] !h-[1.75rem]" />
        </a>
        <VsfNavbarTopActionGroup>
          <VsfNavbarTopAction v-if="!permanent" icon @click="$emit('update:modelValue', false)">
            <VsfIconClose />
          </VsfNavbarTopAction>
        </VsfNavbarTopActionGroup>
      </VsfNavbarTop>
